      ...
      <div ref={issueRef} onClick={onCloseForm} className='fixed inset-0 bg-gray-100 bg-opacity-10 backdrop-blur-sm flex justify-center items-center'>
        <div className='flex flex-col gap-5 text-black'>
          <div className='relative w-full max-w-md bg-white rounded-xl shadow-2xl p-6'>
            <button onClick={closeIssuePage} className='absolute top-3 right-3 text-gray-500 hover:text-black'><X size={30} /></button>
            <form className="w-full flex flex-col justify-start" onSubmit={handleSubmit} >
              <h4 className='font-medium text-sm'>{editingIssue ? 'Edit Issue' : 'Add Issue'}</h4>
              <div className='mt-4'>
                <label htmlFor="title">Title</label>
                <input ref={titleRef} defaultValue={editingIssue?.title || ""} className='w-full px-4 py-3 border border-gray-300 text-black rounded-md mb-5 mt-2' placeholder='Enter the issue Title' type='text' required />
                <label htmlFor="description">Description</label>
                <textarea ref={descriptionRef} defaultValue={editingIssue?.description || ""} className='px-3 py-3 rounded-md w-full border border-gray-300' rows={2} placeholder='description' name="description" id='project-description'></textarea>
